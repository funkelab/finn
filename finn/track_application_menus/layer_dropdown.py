...
class LayerDropdown(QComboBox):
    """QComboBox widget for selecting a finn.layers.Layer of a specific type"""

    layer_changed = Signal(str)  # signal to emit the selected layer name

    def __init__(self, viewer: finn.Viewer, layer_type: tuple):
        super().__init__()
        self.viewer = viewer
    ...
    def _update_dropdown(self) -> None:
        """Update the list of options in the dropdown menu whenever the list of layers is
        changed"""
        ...
        # In case the currently selected layer is one of the available items, set it
        # again to the current value of the dropdown.
