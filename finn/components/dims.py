...
class Dims(EventedModel):
    ...
    def _check_dims(cls, values):
        ...
        updated['point'] = tuple(
            np.clip(pt, rng.start, rng.stop) for pt, rng in zip(point, updated['range'], strict=False)
        )
    ...
    def current_step(self, value):
        self.point = tuple(
            rng.start + point * rng.step for point, rng in zip(value, self.range, strict=False)
        )

    @property
    def thickness(self) -> tuple[float, ...]:
        return tuple(
            left + right for left, right in zip(self.margin_left, self.margin_right, strict=False)
        )
