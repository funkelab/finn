...
def convert_napari_to_finn(napari_name: str, finn_name: str):
    """Convert `napari_name` to `finn_name`, ensuring parent packages are linked too."""
    module = importlib.import_module(finn_name)
    sys.modules[napari_name] = module

    # Attach to parent package if there is one
    if "." in napari_name:
        parent_name, child_name = napari_name.rsplit(".", 1)
        parent_module = sys.modules.get(parent_name)
        if parent_module is None:
            # If parent wasn't imported yet, create a dummy package
            import types
...
# Map napari imports to finn equivalents
...
sync_filters = {
    ...
    TrackPoints: {
        "forward_exclude": {"data"},  # we will sync data separately on TrackPoints as we
        # need finer control
