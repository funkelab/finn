...
class Graph(Layer):
    ...
    def __init__(
        ...
        *,
        ...
        edge_features=None,
        edge_feature_defaults=None,
        properties=None,
        edge_properties=None,
        property_choices=None,
        edge_property_choices=None,
        ...
        self._edge_feature_table = _FeatureTable.from_layer(
            features=edge_features,
            feature_defaults=edge_feature_defaults,
            properties=edge_properties,
            property_choices=edge_property_choices,
            num_data=len(self._edges_data),
        )
        edge_color_properties = (
            self._edge_feature_table.properties()
            if len(self._edges_data)
            else self.edge_feature_table.currents()
        )
        self._edge = ColorManager._from_layer_kwargs(
            n_colors=len(data._edges_buffer),  # TODO:
            colors=edge_color,
            continuous_colormap=edge_colormap,
            contrast_limits=face_contrast_limits,
            categorical_colormap=edge_color_cycle,
            properties=edge_color_properties,
        )
    ...
    def edge_color_cycle(
        self, edge_color_cycle: Union[np.ndarray, cycle]
    ) -> None:
    ...
    @property
    def edge_contrast_limits(self) -> Union[None, tuple[float, float]]:
        """None, (float, float) : clims for mapping the edge_color
        colormap property to 0 and 1
        """
    ...
    def edge_color_mode(self, edge_color_mode):
        color_mode = ColorMode(edge_color_mode)
        color_manager = self._edge
        ...
        if color_mode == ColorMode.DIRECT:
            ...
            if color_property == '':
                if self.features.shape[1] > 0:
                    new_color_property = next(iter(self.edge_features))
                    color_manager.color_properties = {
                        'name': new_color_property,
                        'values': self.edge_features[new_color_property].to_numpy(),
    ...
    def _set_color_mode(
        ...
        """Set the face_color_mode, border_color_mode, or edge_color_mode property
        ...
        attribute : str in {'border', 'face', 'edge'}
    ...
    @property
    def _edges_data(self) -> np.ndarray:
        return self._data._edges_buffer
    ...
    def _view_edge_color(self) -> np.ndarray:
        """Get the edge colors of the edges in view

        Returns
        -------
        view_edge_color : (E x 4) np.ndarray
            RGBA color array for the border colors of the E edges in view.
            If there are no edges in view, returns array of length 0.
        """
        return self.edge_color[self._edges_indices_view]
